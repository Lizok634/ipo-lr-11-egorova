...
transport_company = TransportCompany("My transport company") # Глобальные переменные для хранения данных
...
def show_vip_clients():
    ...
    with dpg.window(label="VIP клиенты", modal=True, width=600, height=400, tag="vip_clients_window"):  # Создаем новое окно "VIP клиенты"
        with dpg.table(header_row=True):  # Создаем таблицу с заголовком
            dpg.add_table_column(label="Имя клиента")  # Добавляем колонку "Имя клиента"
            dpg.add_table_column(label="Вес груза")  # Добавляем колонку "Вес груза"
            for client in filter(lambda c: c.is_vip, transport_company.clients):
...
def show_loaded_vehicles():
    ...
    loaded_vehicles = list(filter(lambda v: v.current_load > 0, transport_company.vehicles))  # Фильтрация транспортных средств с загрузкой больше нуля
...
def save_vehicle():
    ...
    if capacity.isdigit() and int(capacity) > 0:  # Проверка корректности введенной грузоподъемности
        ...
        transport_company.add_vehicle(vehicle)  # Добавление транспортного средства
...
# Создаем окно с VIP клиентами
    with dpg.window(label="VIP клиенты", modal=True, width=600, height=400, tag="clients_window"):  # Создаем новое окно "VIP клиенты"
        ...
        with dpg.table(header_row=True):  # Создаем таблицу с заголовком
            ...
            for client in filter(lambda c: c.is_vip, transport_company.clients):  # Проходим по всем VIP клиентам
                with dpg.table_row():  # Добавляем строку в таблицу для каждого VIP клиента
                    dpg.add_text(client.name)  # Имя клиента
                    dpg.add_text(str(client.cargo_weight))  # Вес груза клиента
...
def export_results():
    data = {
        "clients": [{"name": c.name, "cargo_weight": c.cargo_weight, "is_vip": c.is_vip} for c in transport_company.clients],  # Список клиентов с их данными
        "vehicles": [
            {
            ...
            } for v in transport_company.vehicles  # Проходим по всем транспортным средствам
        ]
    }
...
def optimize_cargo_distribution():
    transport_company.distribute_cargo()  # Распределяем грузы по транспортным средствам
...
def show_about():
    ...
    with dpg.window(label="О программе", width=800, height=600, modal=True, tag="about_window"):  # Создание нового окна "О программе"
        dpg.add_text("Лабораторная работа номер 12", color=[255, 105, 180])  # Добавление текста "Лабораторная работа номер 12"
        dpg.add_text("Вариант: 4", color=[255, 105, 180])  # Добавление текста "Вариант: 4"
        dpg.add_text("Разработчик: Чайковская Елена Витальевна",  color=[255, 105, 180])  # Добавление текста "Разработчик: Чайковская Елена Витальевна"
        dpg.add_button(label="Закрыть", callback=lambda: dpg.delete_item("about_window"))  # Добавление кнопки "Закрыть" с розовым цветом


def setup_fonts():
    with dpg.font_registry():  # Создание реестра шрифтов
        with dpg.font("C:/Windows/Fonts/InkFree.ttf", 20) as default_font:  # Добавление шрифта Arial размером 20
...
def optimize_cargo_distribution():
    transport_company.optimize_cargo_distribution()  # Вызываем метод для распределения грузов
...
def main_window():
    with dpg.window(label="Основное окно", width=1000, height=800):  # Создаем основное окно
